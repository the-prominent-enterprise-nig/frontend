...
export default function CreditMemosList() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await CreditMemos.list(search ? { search } : undefined)
  ...
  const voidMemo = async (id: string) => {
    ...
    setVoiding(null)
    if (!res.success) {
      setError(res.message || res.error || 'Failed to void credit memo')
      return
    }
    load()
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold">Credit Memos</h2>
          <p className="text-sm text-gray-500">
            Every credit memo issued against an AR invoice — manual or auto-created from an approved
            POS return.
          </p>
        </div>
        <button
                          ...
                          <table className="w-full text-xs">
                            <thead className="text-gray-500">
                              <tr>
                                ...
                                <th className="px-2 py-1 text-right">Deduction</th>
                              </tr>
                            </thead>
