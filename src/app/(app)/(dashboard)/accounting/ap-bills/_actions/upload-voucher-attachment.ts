...
/**
 * Scenario 10 (Purchasing & AP) Part 4 — uploads a voucher supporting
 * document (receipt, approval memo, etc.) directly to the backend, bypassing
 * the JSON-only `api` client the same way uploadRfsForm() does for RFS forms.
 * Caller still has to POST /file-attachments separately to link the
 * returned file id to the bill (that call is plain JSON, so it stays
