...
import { useSalesQuotas } from '../_hooks/useSalesQuotas'
...
function GrainBadge({ grain }: { grain: SalesQuota['grain'] }) {
...
// Inverted vs. a spending-quota bar: for a sales TARGET, low progress is the
// concerning state and reaching/exceeding 100% is the good outcome.
function UsageBar({ usedPct }: { usedPct: number }) {
  const pct = Math.min(usedPct, 100)
  const color = usedPct >= 100 ? 'bg-emerald-500' : pct >= 50 ? 'bg-amber-400' : 'bg-red-500'
  return (
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-prominent-purple-900">Sales Targets</h1>
          <p className="mt-1 text-sm text-zinc-500">
            Track branch revenue against a target — reporting only, nothing is blocked.
          </p>
        </div>
        {canManage && (
          <button
      ...
      <div className="mb-6 rounded-xl border border-zinc-200 bg-white p-5">
        ...
        {isUsageLoading ? (
          <div className="flex items-center gap-2 text-sm text-zinc-400">
            <Loader2 className="h-4 w-4 animate-spin" /> Loading actuals…
          </div>
        ) : usage?.quota ? (
          <div className="space-y-2">
            <div className="flex items-end justify-between">
              <span className="text-2xl font-semibold text-prominent-purple-900">
                {fmt(usage.currentActual)}
              </span>
              <span className="text-sm text-zinc-500">
                of {fmt(Number(usage.quota.targetAmount))}
              </span>
            </div>
            <UsageBar usedPct={usage.usedPct} />
            <div className="flex justify-between text-xs text-zinc-400">
              <span>{usage.usedPct.toFixed(1)}% reached</span>
              <span>{fmt(Math.max(usage.remaining ?? 0, 0))} to go</span>
            </div>
          </div>
        ) : (
          <p className="text-sm text-zinc-400">
            No active sales target configured for the current period.
          </p>
        )}
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-sm text-zinc-500">No sales targets configured</p>
            {canManage && (
              <p className="mt-1 text-xs text-zinc-400">
                Create a target to track branch revenue performance
              </p>
            )}
          </div>
        ) : (
