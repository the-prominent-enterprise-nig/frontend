...
export default function ReceiptBrandingClient({ isBranchManager, ownBranch }: Props) {
  const { branchId: contextBranchId } = usePosBranchContext()

  // The branch switcher is only ever visible to Business Owner (Branch
  // Manager is always forced onto their own branch below), so "All
  // Branches" selected here can only mean Business Owner — editing the
  // company-wide default is deliberately not reachable by Branch Manager
  // through this page.
  if (!isBranchManager && contextBranchId === null) {
    return <TenantDefaultEditor />
  }

  return <BranchOverrideEditor isBranchManager={isBranchManager} ownBranch={ownBranch} />
}

// ─── All Branches: company-wide default ───────────────────────────────────────

function TenantDefaultEditor() {
  ...
  async function handleLogoUpload(file: File) {
    ...
    setBranding((b) => (b ? { ...b, receiptLogoUrl: res.data!.logoUrl } : b))
  }
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              ...
              receipts. A branch can override any of these just for itself by selecting it above.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <ReceiptPreviewPanel
            ...
            logoUrl={branding?.receiptLogoUrl ?? null}
...
// ─── A specific branch: override of the company default ───────────────────────

function BranchOverrideEditor({
  isBranchManager,
  ownBranch,
}: {
  isBranchManager: boolean
