...
/**
 * Uploads a credit-application supporting document directly to the backend.
 * Bypasses the browser `/api/*` proxy and the JSON-only `api` client — neither
 * can carry a multipart body — mirroring crm/customers/_actions/upload-id-document.ts's
 * identical direct-to-API_URL path for the same reason.
 */
export async function uploadCreditApplicationFile(
  formData: FormData
