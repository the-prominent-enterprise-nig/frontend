      ...
      >
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
          ...
          <div>
            ...
            <div className="hidden grid-cols-[140px_110px_minmax(120px,1fr)_minmax(120px,1fr)_32px] gap-2 px-2.5 pb-1 text-[11px] font-medium uppercase tracking-wide text-zinc-400 md:grid">
            ...
            <div className="space-y-2">
              {fields.map((field, index) => {
                const rowError =
                  errors.lines?.[index]?.componentType || errors.lines?.[index]?.amount
                return (
                  <div key={field.id} className="rounded-lg border border-zinc-200 bg-zinc-50 p-2">
                    <div className="grid grid-cols-[140px_110px_minmax(120px,1fr)_minmax(120px,1fr)_32px] items-center gap-2">
                      <Controller
                        ...
                        render={({ field: f }) => (
                          <select
                            ...
                            className={`${fieldClass} min-w-0 bg-white text-xs`}
                          >
                      ...
                      <Controller
                        ...
                        render={({ field: f }) => (
                          <input
                            ...
                            aria-label="Amount"
                            placeholder="0.00"
                            className={`${fieldClass} min-w-0 text-xs [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none`}
                      ...
                      <Controller
                        ...
                        render={({ field: f }) => (
                          <input
                            ...
                            className={`${fieldClass} min-w-0 text-xs`}
                          />
                        )}
                      />
                      ...
                      {fields.length > 1 ? (
                        <button
                          ...
                          className="flex h-8 w-8 items-center justify-center rounded p-1 text-zinc-400 hover:bg-red-50 hover:text-red-500"
                        >
                    ...
                    {rowError && (
                      <div className="mt-1 space-y-0.5 text-xs text-red-600">
                        {errors.lines?.[index]?.componentType && (
                          <p>{errors.lines[index]?.componentType?.message}</p>
                        )}
                        {errors.lines?.[index]?.amount && (
                          <p>{errors.lines[index]?.amount?.message}</p>
                        )}
                      </div>
                    )}
                  </div>
