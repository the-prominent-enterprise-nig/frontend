...
import { CreateProcurementQuotaSchema } from '@/src/schema/inventory/procurement-quotas'
import type { CreateProcurementQuotaValues } from '@/src/schema/inventory/procurement-quotas'

type Props = {
  onClose: () => void
  onSubmit: (data: CreateProcurementQuotaValues) => Promise<any>
...
export function CreateQuotaModal({ onClose, onSubmit, isSubmitting }: Props) {
  const {
    ...
    control,
    formState: { errors },
  } = useForm<CreateProcurementQuotaValues>({
    resolver: zodResolver(CreateProcurementQuotaSchema),
    defaultValues: { fiscalYear: new Date().getFullYear(), grain: 'annual' },
  })

  const handleFormSubmit = async (data: CreateProcurementQuotaValues) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-zinc-200 px-6 py-4">
          <h2 className="text-base font-semibold text-zinc-900">New Spending Quota</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 px-6 py-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Any notes about this quota..."
            />
          ...
          <div className="flex items-center justify-end gap-3 border-t border-zinc-100 pt-4">
            ...
            >
              {isSubmitting ? 'Creating…' : 'Create Quota'}
            </button>
          </div>
        </form>
      </div>
