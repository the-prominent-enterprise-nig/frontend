...
export async function updateProcurementQuota(
  id: string,
  data: UpdateProcurementQuotaValues
  ...
  const result = await api.patch<ProcurementQuota>(`/procurement/quotas/${id}`, data)
  ...
  if (!result.success) {
    ...
    return {
      success: false,
      error: errStr || 'Failed to update quota',
      message: msg || errStr || 'Failed to update quota',
    }
  }

  revalidatePath('/inventory/procurement-quotas')

  return { success: true, data: result.data, message: 'Spending quota updated successfully' }
}
