...
type Props = {
  ...
  gaps: ReturnGap[]
  /** Whether there is a sale in play to return against — a named account, or
   *  a walk-in's receipt. Not the same as having a customer: a walk-in
   *  legitimately has none, and reporting that as the outstanding problem
   *  sent people looking for a field they were right to leave empty. */
  hasSale: boolean
...
export default function ReturnActionBar({
  ...
  return (
    <div className="sticky bottom-0 z-20 shrink-0 border-t border-[#e4e4e9] bg-white shadow-[0_-8px_24px_-18px_rgba(20,20,30,.35)]">
      {/* Same 1560 gutter as the header and the scrolling body, so the total
          and the Post button sit under the ledger they belong to rather than
          out at the window's edge. */}
      <div className="mx-auto flex w-full max-w-[1560px] flex-wrap items-center justify-between gap-3.5 px-[14px] py-3 min-[1080px]:px-5">
        <div className="flex min-w-0 flex-col gap-0.5">
          <span
            className={`text-[11.5px] ${
              hasSale && settlement.lines > 0 && blocked ? 'text-[#b42318]' : 'text-[#5b5b6b]'
            }`}
          >
            {note}
          </span>
