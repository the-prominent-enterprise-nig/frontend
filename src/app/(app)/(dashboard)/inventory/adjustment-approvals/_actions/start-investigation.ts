...
export async function startInvestigation(id: string): Promise<ApiResponse<unknown>> {
  ...
  const result = await api.post(`/inventory/adjustments/${id}/start-investigation`)

  if (!result.success) {
    return {
      success: false,
      error: result.error ?? 'Failed to start investigation',
      message:
