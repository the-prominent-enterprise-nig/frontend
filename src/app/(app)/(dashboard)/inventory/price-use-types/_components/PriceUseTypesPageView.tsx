...
import { ArrowLeft, Plus, RefreshCw, Tags, Pencil, Trash2 } from 'lucide-react'
import { hasPermission } from '@/src/hooks/usePermission'
import { INVENTORY_PERMISSIONS } from '@/src/libs/guards/inventory-permissions'
import type { SessionUser } from '@/src/libs/guards/permission'
import { usePriceUseTypes } from '../_hooks/usePriceUseTypes'
import PriceUseTypeModal from './PriceUseTypeModal'
import type { PriceUseType, PriceUseTypeFormValues } from '@/src/schema/inventory/price-use-types'

export default function PriceUseTypesPageView({ session }: { session: SessionUser }) {
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          <Link
            ...
            className="mb-2 inline-flex items-center gap-1.5 text-sm font-medium text-zinc-500 hover:text-prominent-purple-700"
          >
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 md:text-3xl">Price Use Types</h1>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-zinc-500">
              This removes <strong>{deletingType.name}</strong>. Price lists still using it must be
              reassigned first.
            </p>
