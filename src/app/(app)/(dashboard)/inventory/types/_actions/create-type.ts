...
export async function createType(input: unknown): Promise<ApiResponse<unknown>> {
  ...
  const result = await api.post('/inventory/classification/types', parsed.data)

  if (!result.success) {
    return {
      success: false,
      error: result.error || 'Failed to create type',
      message: result.message || 'Failed to create type',
    }
  }

  revalidatePath('/inventory/catalog')

  return { success: true, data: result.data, message: 'Type created successfully' }
}
