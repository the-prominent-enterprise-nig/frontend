...
export async function updateType(id: string, input: unknown): Promise<ApiResponse<unknown>> {
  ...
  const result = await api.patch(`/inventory/classification/types/${id}`, parsed.data)
  ...
  return { success: true, data: result.data, message: 'Type updated successfully' }
}
