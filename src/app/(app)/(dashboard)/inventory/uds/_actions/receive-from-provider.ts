...
export async function receiveFromProvider(
  ...
  if (!can(session, INVENTORY_PERMISSIONS.UDS_MANAGE)) {
    return {
      success: false,
      error: 'Forbidden',
      message: 'You do not have permission to receive a UDS back',
    }
  }

  const parsed = ReceiveFromProviderFormSchema.safeParse(input)
  if (!parsed.success) {
    return {
      success: false,
  ...
  return { success: true, data: result.data, message: 'Unit received back from the service centre' }
}
