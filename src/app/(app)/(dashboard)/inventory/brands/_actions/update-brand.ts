...
export async function updateBrand(id: string, input: unknown): Promise<ApiResponse<unknown>> {
  ...
  const result = await api.patch(`/inventory/classification/brands/${id}`, parsed.data)

  if (!result.success) {
    return {
      success: false,
      error: result.error || 'Failed to update brand',
  ...
  return { success: true, data: result.data, message: 'Brand updated successfully' }
}
