...
/**
 * Uploads a customer's scanned ID document directly to the backend.
 * Bypasses the browser `/api/*` proxy and the JSON-only `api` client — neither
 * can carry a multipart body — mirroring uds/_actions/upload-rfs-form.ts's
 * identical direct-to-API_URL path for the same reason.
 */
export async function uploadIdDocument(formData: FormData): Promise<ApiResponse<UploadedFile>> {
