...
1. **PR → PO with quota enforcement — CONFIRMED-WORKING.** Full pipeline: `backend/src/inventory/services/{purchase-request,purchase-order,procurement-quota}.service.ts`, controllers, and frontend pages under `inventory/{purchase-requests,purchase-orders,procurement-quotas}/`. `ProcurementQuota` is a distinct Prisma model (`schema.prisma:6053`, line drifted from unrelated later growth — was `:4341` when first cited); `PurchaseOrderService` calls `quotaService.enforceQuota(...)` on PO create/update/approve (`purchase-order.service.ts:269, 397, 475`, was `:139, 243, 297`). Suppliers live in Inventory as intended: `model Supplier` (`schema.prisma:1519`, was `:1104`), `backend/src/supplier/supplier.service.ts`, UI at `inventory/suppliers/`.
2. **PO ↔ RR match — CONFIRMED-WORKING.** `stock.service.ts::receiveStock` links receipt lines to `PurchaseOrderLine` via `purchaseOrderLineId` (`GoodsReceiptLine`, `schema.prisma:4381`, was `:3161`), increments `receivedQuantity` per PO line, and drives PO status through `partially_received`/`fully_received` (`stock.service.ts:404-467`) — multiple partial RRs against one PO/PO line are correctly supported.
3. **PO/PR destination is now a required "Warehouse" field, restricted to the 2 real warehouses — added 2026-08-12 by Scenario 27 (Warehouse Tier Correction), not this scenario's own work.** `CreatePoModal.tsx`/`ConvertPrToPoModal.tsx` both require picking PANAY or NEGROS (`purchase-order.dto.ts` carries `warehouseId` on all four DTOs); `ReceiveAgainstPoModal.tsx`'s "Destination Warehouse" locks once the PO already has one. Noted here for completeness since it's a real, current part of the PR→PO→RR flow this scenario describes, even though Scenario 27 did the work.
