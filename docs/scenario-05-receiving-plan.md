...
- **Cost-visibility inventory-wide expansion deferred** — confirmed with the developer to scope this pass to the Receiving flow only. The updates doc's broader ask (also restrict cost on Item Master list/detail, Costing/Valuation, Bundles, PO screens — 14 files total) is still open; worth its own pass, likely against Scenario 16 (Item Master Governance) rather than this one.
...
**For this scenario, I have done:** the two follow-ups the 2026-07-31 log flagged as deferred, closing both remaining checklist items.

- **Cost-view expanded beyond the Receiving flow (Parts 1-4)** — the 14-file gap flagged 2026-07-31 ("beyond the RR itself... Item Master, Costing/Valuation, Bundles, PO screens"). Renamed the permission `inventory:receive:cost-view` → `inventory:cost:view` (same rule now applies everywhere unit cost appears, not just Receiving) and extended enforcement:
  - **Part 1 — Item Master**: `items.service.ts` strips `costPrice` on `findAll`/`findOne`/`getChangeHistory`/`getItemLedger` for a caller without cost-view; matching UI gating in `ItemMasterTable.tsx`, `Item360Drawer`/`OverviewTab.tsx`, `EditItemModal.tsx`. Item creation was initially blocked entirely for non-cost-view roles (mirroring "cost is required"), but `costPrice` is actually optional at the DTO level — the block broke Scenario 16's item-master-governance draft-creation flow, so this was corrected to "create allowed, cost silently stripped" (same treatment as Receiving), confirmed with the developer.
  - **Part 2 — Bundles**: inherited Part 1's fix for free (bundles are `Item` rows with `isBundle: true` through the identical endpoints) — verified this holds rather than assumed, with dedicated new test coverage. No bundle-specific backend code needed.
  - **Part 3 — Costing/Valuation + Item Revaluation**: `costing.service.ts`'s `getValuation`/`previewCogs`/`issueStock`/`getRevaluationHistory`/`createItemRevaluation`, plus the separate `/inventory/reports/valuation` endpoint, now respect cost-view. Valuation/revaluation-history strip cost fields but keep item/qty/reason/notes (mirroring Item Master's per-field pattern); `issueStock` still succeeds with cost stripped from the response only (system-computed, not caller-submitted); `createItemRevaluation` is blocked outright (`newCost` is genuinely required, no optional path). Frontend: Costing page's Valuation tab now requires cost-view in addition to the existing read permission (reusing the page's own "no permission" notice); Issue Stock modal and Revaluation page adapt to null cost fields.
  - **Part 4 — Purchase Orders**: `PoDetailModal.tsx` had no cost-view gating at all (the biggest single PO gap) — fixed, plus `purchase-order.service.ts`'s `findAll`/`findOne`/`getDocument` now strip per-line `unitPrice`/`lineTotal`/`srp`/`discountType`/`discountValue`/`discountedCost`, while header `subtotalAmount`/`totalAmount` stay visible to everyone (developer-confirmed — a Stock Controller still needs to know what a PO totals to). The PDF export (`renderPoBody` in `PurchaseOrderList.tsx`) drops the Unit Price/Line Total columns entirely rather than printing misleading ₱0.00 rows.
  - **Cross-scenario fix found along the way**: restoring Accountant's inventory access (needed so Accountant can actually reach these cost-viewing screens) initially also granted it `items:approve`, which would have let the same role both confirm tax/GL mapping and give final item approval — defeating Scenario 16's maker-checker design. Excluded `items:approve` from Accountant's grant, same `modulePermIdsExcept` pattern already used for Stock Controller/Branch Manager's `cost:view` exclusion.
  - **Second wildcard-bypass leak found via testing**: Cashier held `inventory:cost:view` through a catch-all "every other inventory read" grant that was supposed to exclude cost/margin fields but didn't list `cost:view` explicitly — fixed.
- **Part 5 — Freebies on "Receive against PO"** — closes the other 2026-07-31 deferral. `ReceiveAgainstPoModal.tsx` was hard-locked to exactly the PO's own line items; added an "Add Freebie Item" action (live item search, no `purchaseOrderLineId`, `isFreebie: true` forced) so a supplier-given free extra unit that was never on the original order has somewhere to go, without needing a separate standalone receipt. No backend changes were needed — `POST /inventory/stock/receive` already supported mixing PO-tied and untied lines in one call (the standalone Receiving form already used this); this part only unlocked the UI path and proved the existing mixed-line mechanic holds for it.

**Worth flagging:**

- Both sub-items the 2026-07-31 log left open ("cost-view beyond Receiving" and "freebies on Receive-against-PO") are now closed — Scenario 5 has no known open gaps as of this entry.
- New coverage: 5 new backend e2e specs (`inventory-bundle-cost-visibility`, `inventory-costing-cost-visibility`, `inventory-po-cost-visibility`, `inventory-po-receiving-freebie`, plus Item Master coverage folded into existing specs) — 26 new tests; 5 new frontend Playwright specs — 23 new tests. Fixed 3 pre-existing tests whose assumptions this session's RBAC changes legitimately broke (`item-master-governance`, `inventory-phase5-valuation`'s persona, `procurement-closeout`'s negative-permission persona). Full regression sweep across 11 backend suites (~95 tests) and 5 frontend specs (23 tests): all green. Both repos' builds/type-check/lint clean. Manually re-confirmed live in-browser for both Business Owner and Stock Controller across all 5 parts.
- Two pre-existing, unrelated bugs surfaced while regression-testing `procurement-closeout.e2e-spec.ts` and left untouched (out of scope): a branch-scoped procurement quota not actually enforcing (`resolveQuota`'s `branchId` matching), and a write-off test calling a `/inventory/adjustments/write-off` route that no longer exists (superseded by the UDS/Repair Transfer flow, per the 2026-07-27 log entry above).
