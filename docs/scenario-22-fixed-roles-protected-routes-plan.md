...
4. **Two overlapping "inventory" roles exist in the seed with no documented distinction** — `Stock Controller` (curated permission list, `seed.ts:2424-2432`) and a separately-seeded `Inventory` role (broader `inventory:*` wildcard, `seed.ts:2511-2519`). This is exactly the ambiguity in the request itself ("inventory(person?)") — the system currently has two answers, not one, and needs a decision rather than a silent pick.
...
### 4. Reconcile `Stock Controller` vs `Inventory`

**Fix**: confirm with the business which is the real fixed role for this tier; retire or explicitly repurpose the other. Don't leave both live with no documented distinction — the current state is genuinely ambiguous, not a design choice.
...
4. **`Stock Controller` vs `Inventory`** — which is the real fixed-role-list member (gap 4 above); needed before the fixed six-role list can be considered final.
