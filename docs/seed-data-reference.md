...
| `TN-FURN-SET-001`   | TV Console Furniture Set | Yes             | No           | **Drifted from the "6 total" below** — as of 2026-07-20 there are 48 `in_stock` serials per warehouse (144 total), all named `FURNSET-WH-0N-BULK-0NN` (e.g. `FURNSET-WH-01-BULK-001`), from repeated manual bulk-serial-registration testing since this doc was written. Query `serial_numbers` directly for the current exact set rather than trusting the table below. |
| `TN-AC-SPLIT-1_5HP` | Split-Type Aircon 1.5HP  | Yes             | **Yes**      | **6 total, all `in_stock`** — see table below. Added this session as the dual-serial demo. Not re-verified as of 2026-07-20 — spot-check before relying on it.                                                                                                                                                                                                           |

### Furniture Set serials (`TN-FURN-SET-001`) — stale, see drift note above
