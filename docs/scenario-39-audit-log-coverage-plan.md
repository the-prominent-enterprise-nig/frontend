# Scenario 39 — Full Transaction Audit Log Coverage — Gap Analysis & Closing Plan
...
- Full accumulated Scenario 39 regression across all 12 `accounting-audit-log-*.e2e-spec.ts` files run together via `--runInBand`: 52/52 passing.
...
- Full accumulated Scenario 39 regression, all 13 `accounting-audit-log-*.e2e-spec.ts` files plus the pre-existing Collections/Collector/Installment-Account suites, run together via `--runInBand`.
...
## Implementation Log — 2026-08-24 (Closing Gap 5 — Inventory remainder, fully closed)

**For this scenario, I have done:**

- **Closing Gap 5, all 9 sub-parts of Inventory remainder**, one part per sub-module (Part 8 further split into 7 sub-parts per developer's confirmed choice), every action logged via the established direct-call `AccountingAuditLogService.write()` pattern (the `@AuditLog` decorator/interceptor from Gap 1 remains unused by any real controller, same open question noted 2026-08-17):
  - **Part 1 — Adjustments** (`inventory:stock-adjustment`): create/confirm/investigate/approve/reject/delete(withdraw), each carrying `metadata.items` (line-level item name/SKU summary).
  - **Part 2 — Transfers** (`inventory:transfer`): create/requestFromPos/approveManager/rejectManager/approveHq/rejectHq/accept/reject/dispatch/receive/cancel (11 actions).
  - **Part 3 — Stock Counts** (`inventory:stock-count`): create/start/addLine/submit/cancel.
  - **Part 4 — Batches** (`inventory:batch`): create/updateStatus/placeOnHold/releaseHold.
  - **Part 5 — Price Lists** (`inventory:price-list`): create/update/deactivate/approve/reject/resubmit/upsertItems/removeItem.
  - **Part 6 — Purchase Requests** (`inventory:purchase-request`): create/update/submit/approve/reject/cancel.
  - **Part 7 — SKU Reservations** (`inventory:sku-reservation`): create/fulfil/requestCancel/approveCancel/rejectCancel. Also found and fixed an unrelated gap surfaced mid-part: the Reservations page (fulfil/cancel flows) existed and was fully built but had no sidebar nav entry — added one (`SideBar.tsx`, gated on the page's own `SKU_RESERVATIONS_READ` permission).
  - **Part 8 — Item edits** (`inventory:item`, 26 mutating actions across 7 sub-parts — the largest controller instrumented this scenario): 8a Core (create/update/remove/receiveInitial/bulkImport), 8b Governance (updateLifecycle/submitForReview/confirmAccounting/rejectAccounting/approveItem/rejectItem), 8c Variants (createVariant/updateVariant/removeVariant), 8d Barcodes (createBarcode/removeBarcode/generateBarcode/bulkGenerateBarcodes/addSecondaryBarcode), 8e Bundles (addBundleComponent/removeBundleComponent), 8f Categories (assignCategory), 8g Substitutes/Tags (addSubstitute/removeSubstitute/addTag/removeTag). Developer-confirmed (2026-08-24): keep this coverage even where it overlaps `ItemChangeLog`'s narrower item-field history, rather than dedupe against it.
  - **Part 9 — Costing/Revaluation** (`inventory:costing`): upsertConfig/updateItemCostingMethod/issueStock/createRevaluation/createItemRevaluation. Two of the five (`upsertConfig`, `createRevaluation`) are tenant-wide (logged against the `InventoryCostingConfig` row's own id, since it's `@unique` per tenant, not per-item); the other three are item-scoped. `landed-cost.controller.ts` and `sku-reservations.controller.ts`'s own separate `reservations.controller.ts` sibling were confirmed out of scope for Gap 5 (developer-confirmed 2026-08-17/24) and left untouched.
  - 15 new `test/accounting-audit-log-inventory-*.e2e-spec.ts` files across the 9 parts, 77 new e2e tests, all passing. `tsc --noEmit` and `eslint --fix` clean throughout.
- **New UI work beyond this doc's original scope, developer-requested mid-implementation**: the Settings → Audit Logs Resource column was unreadable to a business owner (raw `resourceType` strings, clipped actor/resource UUIDs, no way to tell _which item_ an adjustment/count/reservation touched). Fixed in `AuditLogsSection.tsx`: `formatResourceType()` + `MODULE_LABEL_OVERRIDES` render a human module label + resource name instead of the raw `module:resource` string; a new `metadata.items: string[]` convention (`"ItemName (SKU)"`, populated across every Gap 5 action touching one or more items) renders as a subtitle under the resource name — deliberately always shown regardless of whether it changed, since it's excluded from the before/after diff view; actor/resource ID truncation (`.slice(0,12)…`) removed in favor of `break-all` wrapping, per developer request not to clip IDs.
- **Pre-existing issues found via `git stash`-verified re-runs (flagged, not fixed — confirmed identical failures against the unmodified controller in every case)**:
  - `stock-transfer-hq-approval.e2e-spec.ts`: "Branch Manager cannot approve/reject at HQ (403)" now gets 200.
  - `notifications-purchase-request-order.e2e-spec.ts`: stale fixture missing now-required `supplierId`/`unitPrice`.
  - `sku-reservations.e2e-spec.ts`: "Stock Controller has no create permission (403)" now gets 201.
  - `inventory:bundles:delete` was never added to the seeded permission catalog (only `bundles:read`/`:create` were), despite being required by `removeBundleComponent()`'s `@RequirePermissions` — no role except Business Owner (permission-check bypass) can currently reach that endpoint.
  - `inventory-catalog-enrichment.e2e-spec.ts`: an image-primary-repromotion assertion and a `/files/upload` 403 for Stock Controller — both unrelated to any Gap 5 action (image handling and file uploads aren't part of any controller touched this scenario).

**Worth flagging:**

- **Closing Gap 5 (Inventory remainder) is now fully closed** — every financially/operationally-material action across Adjustments, Transfers, Stock Counts, Batches, Price Lists, Purchase Requests, SKU Reservations, Item edits, and Costing/Revaluation writes a real before/after `AccountingAuditLog` entry, with human-readable item identification via `metadata.items`.
- Remaining scenario scope: **Closing Gaps 6-7** (CRM beyond Credit & Collections, remaining POS admin) and **Closing Gaps 8-10** (OpenAPI regen — now even more stale, Recent Activity widget allowlist decisions, `AuditLogService`/`AccountingAuditLogService` unit tests) — none started yet.
- The five pre-existing issues above are real gaps independent of this scenario's own scope (RBAC seeding, stale fixtures, image/upload logic) — worth their own tickets, not silently absorbed into Scenario 39's closing-gap list.
