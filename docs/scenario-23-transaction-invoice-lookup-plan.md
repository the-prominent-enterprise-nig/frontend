...
- **Closing Gap 5** (implemented first, per its own "land first" priority note): `createAndPostInstallmentPlan()` now groups a transaction's installment lines by shared `financingTermId` — one `InstallmentSchedule`/`InstallmentAccount`/down-payment/due-date set per distinct term used, not per line. Relaxed the `@unique` constraints on `InstallmentSchedule.posTransactionLineId` and `InstallmentAccount.installmentScheduleId` (schema migration) since a schedule now relates to potentially several `PosTransactionLine`s.
- **Closing Gap 1**: `TransactionsService.findOne()` now includes the linked `arInvoice` (charge) and every installment schedule's per-due-date `arInvoice` rows, returned as a flat `invoices` array on the POS transaction detail screen — rendered in a new Invoices section on `TransactionsList.tsx`'s detail modal.
- **Closing Gap 2**: `getInstallmentSchedules()` now also includes `posTransactionLine.item` (name + brand) and `installmentAccount.ppd` (the rebate). Display went through a UX redesign beyond the original ask, per developer feedback mid-implementation: Customer360's Installment Plans section now shows a collapsed per-schedule summary (product + term + total) that opens a detail modal on click — mirroring the POS transaction detail modal's style — rather than the originally-planned dense inline listing, on the reasoning that a cashier shouldn't see a customer's full AR ledger inline the way a Business Owner can.
- **Closing Gap 3**: `TransactionsList.tsx`'s search now matches a transaction number, its charge invoice number, or any of its installment due-date invoice numbers — resolved via `TransactionsService.findAll()`'s unified `OR` lookup, not three separate fields.
- **Closing Gap 4**: `ARInvoicesService.findAll()`'s search replaced the old accidental `description`-contains match with a structured lookup through `PosTransaction`/`InstallmentScheduleLine`, returning the full set of a transaction's invoices. The AR Invoices screen had no search input at all before this — added one.
- Both new/touched search boxes (POS Transactions, AR Invoices) auto-search ~400ms after typing stops, no Apply click required (developer feedback mid-implementation) — the Type/Status/Date filters on POS Transactions still gate on Apply since those are usually set together before submitting.

**Worth flagging:**

- A significant amount of unplanned work landed alongside this scenario, surfaced by the developer's own live testing against a reseeded DB:
  - **RBAC**: Cashier's `pos:*` wildcard incorrectly included `pos:transaction:override` (self-approval loophole) and was missing `crm:customers:read` / several `inventory:*:read` grants needed by the checkout flow — fixed in `seed.ts`. Also removed unnecessary `@RequirePermissions` from ~20 read-only reference-data routes across inventory/POS/CRM/files controllers (`pos:sessions:read` was reverted after a real e2e test caught that session/cash data should stay restricted even as a "read").
  - **Dirty-data bug class**: `findAll()` on 6 services (`InstallmentAccountService`, `CreditApplicationService`, `ReminderService`, `SkuReservationsService`, `CustomerAdvancesService`, `ServiceDraftsService`) never filtered out soft-deleted customers, so their records lingered on live list screens indefinitely. Fixed on all 6 (two — `ReminderService`, `ServiceDraftsService` — needed an `OR`-guarded fix since their customer relation is optional).
  - **Credit application numbering**: `generateApplicationNumber()` used `count()`-based sequencing, which permanently breaks the moment any row in the day's range is deleted (recomputes an already-taken number on every retry). Rewrote to max-based sequencing in `credit-application.service.ts`.
  - **Release-form-request approval**: `ReleaseFormRequestsService.approve()` replayed the cashier's original (possibly since-closed) session through `TransactionsService.create()`'s session-must-be-open check, blocking approval of any request left pending past the cashier's shift end. `create()` never actually writes to the session, so added an `allowClosedSession` option used only on this replay path.
  - **Credit application checkout error**: replaced a raw-UUID, backend-jargon error shown directly to cashiers with `describeUnusableCreditApplication()` — a specific, actionable reason (wrong branch / already used / not yet approved / not found).
  - **App-wide "Overdue" status gap**: `ARInvoiceStatus`/`APBillStatus`'s `OVERDUE` value only ever got written during a payment-recording mutation, never recomputed on a read — a fully-unpaid, past-due record just displayed as "Due" forever. Added `src/accounting/shared/invoice-status.util.ts` and applied read-time recomputation across 10 call sites: AR Invoices, AP Bills (incl. all 4 voucher-approval steps), Credit Memos, Supplier Debit Memos, the Customer and Supplier Statement reports, plus this scenario's own POS transaction detail, Customer360 installment modal, and the CRM Installment Account detail page.
  - Credit Applications list: replaced a separate "Open" text link with a directly clickable row, matching this app's established list-row convention.
- None of the above were part of this scenario's original closing-gap list — they're documented here because they were found and fixed live during this scenario's manual testing, not tracked against any other scenario doc. Worth a pass to check whether any should be back-filled into Scenario 17 (installment financing) or a dedicated bug-tracking doc.
