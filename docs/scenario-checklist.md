...
Last verified: 2026-08-04 (Scenario 19 implemented this pass, 4 of 5 closing-gap items closed; Scenario 16 implemented and closed this pass; Scenario 14 closed 2026-08-03; Scenario 20 fully closed 2026-08-03 — NAMIDRe + DAM; Scenario 02 re-verified 2026-08-01; Scenario 05 re-verified 2026-07-31).
...
- [ ] **19 — Stock Count & Inventory Adjustment Approval** — server-side count snapshot, `StockAdjustment` approval chain (submitted → confirmed → investigating → approved/rejected, deferred stock/GL posting until approve()), before/after audit values on `StockLedger`, and non-saleable status (existing `BatchStatus`/`SerialNumberStatus`) surfaced on adjustment lines all implemented (2026-08-04). [plan](./scenario-19-stock-count-adjustment-approval-plan.md)
