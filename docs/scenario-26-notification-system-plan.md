...
- ~~**Purchase Request → Purchase Order** (`purchase-request.service.ts` + `purchase-order.service.ts`) — Stock Controller requests → approver ambiguous in current RBAC (resolves to Branch Manager/Business Owner via the `inventory:*` wildcard; single-tier by design, multi-tier explicitly out of scope elsewhere in the code).~~ **Closed 2026-08-13** — see the same-day follow-up Implementation Log entry below (RBAC ambiguity resolved: Stock Controller's approve/reject access removed, Branch Manager kept).
...
## Implementation Log — 2026-08-13 (same-day follow-up)

After the Phase 1 + Phase 2 closure above, the developer asked what other important POS/Inventory actions still needed notification coverage. An audit of both modules for maker-checker/approval-style state transitions (approve/reject/confirm/cancel/void/dispatch/receive) found 3 genuine gaps beyond the already-scoped catalog — real distinct requester/approver permissions, zero notification wiring — plus the developer separately flagged Purchase Request → Purchase Order (already catalogued as deferred Phase 3, but unblocked enough to close now). All 4 implemented, tested, and confirmed this same pass:

**For this scenario, I have done:**

- **SKU Reservation cancellation** (`sku-reservations.service.ts`) — `requestCancel()`/`approveCancel()`/`rejectCancel()` now notify: the requester's branch's Branch Manager + Business Owner on request (excluding the requester, since the doc's own comment notes a Branch Manager/Business Owner can request their own reservation's cancellation), and exactly the original cancel-requester on resolution. New event types `sku_reservation_cancel_approval_needed`/`sku_reservation_cancel_resolved`. 3 new e2e tests added to the existing `sku-reservation-cancel-refund.e2e-spec.ts`.
- **Price List approval** (`price-lists.service.ts`) — `create()`/`approve()`/`reject()`/`resubmit()` now notify. Added `PriceList.createdById` (new nullable column + migration — this scenario's Closing Gap 8 blocker, now closed for Price List specifically; AP Bills' half of that gap remains open). `allowedBranchIds` is multi-valued (or empty = company-wide) unlike every other workflow in this catalog — notifies each listed branch's Branch Manager individually rather than guessing one, and only Business Owner when company-wide (no wildcard broadcast to every branch). New event types `price_list_approval_needed`/`price_list_resolved`. New spec `notifications-price-list.e2e-spec.ts`, 4 tests.
- **Batch quality-hold disposition** (`batches.service.ts`) — `updateStatus()` (which `placeOnHold()`/`releaseHold()` both route through) now notifies. Added `Batch.heldById`/`heldAt` (new nullable columns + migration — Batch had zero actor tracking before this). A batch has no single branch of its own (`StockBalance` rows can span multiple warehouses) — hold-placed notifies every distinct branch's Branch Manager the batch actually has stock in + Business Owner, excluding the actor; hold-resolved notifies exactly whoever placed the hold. Weakest of the 4 gaps by design (same `inventory:batch:manage` permission gates both sides — not a real two-tier RBAC split, left as-is per the developer's scope confirmation), but still a real "tell whoever placed the hold how it was resolved" need. New event types `batch_hold_placed`/`batch_hold_resolved`. New spec `notifications-batch-hold.e2e-spec.ts`, 3 tests.
- **Purchase Request → Purchase Order** (Closing Gap 6 item, moved out of Phase 3 deferral) — `purchase-request.service.ts`'s `submit()`/`approve()`/`reject()` and `purchase-order.service.ts`'s `create()`/`convertFromPr()`/`approve()` now notify. PR's single approval tier (`PR_APPROVAL_TIERS`, still explicitly single-tier by design) notifies on submit and resolves on the last tier; PO has no separate submit step (`draft` status IS the pending-approval stage) so `create()`/`convertFromPr()` notify directly. New event types `purchase_request_approval_needed`/`purchase_request_resolved`/`purchase_order_approval_needed`/`purchase_order_resolved`. New spec `notifications-purchase-request-order.e2e-spec.ts`, 7 tests.
- **RBAC fix, same shape as Part 9's**: Stock Controller's `inventory:*` wildcard-except grant in `prisma/seed.ts` also covered `purchase-requests:approve`/`:reject` and `purchase-orders:approve` — the same self-approval hole (Stock Controller raises PRs/POs and could also approve/reject its own). Narrowed to exclude those three slugs too; Branch Manager's grant deliberately left untouched (create+approve both intended for that role, unlike the Stock Adjustment split). Applied to the live dev DB directly. Fixed one pre-existing test in `procurement-closeout.e2e-spec.ts` ("403s approving a PO without procurement:purchase-orders:approve, 200s with it") that asserted the old, incorrect Stock-Controller-can-approve behavior as correct — added a `managerToken` actor and pointed the "200s with it" assertion at Branch Manager instead, which is the real, intended holder of that permission.

**Worth flagging:**

- All 6 new/changed migrations (`scenario26_followup_notification_events`, `price_list_created_by`, `batch_held_by`, `scenario26_pr_po_notification_events`) were applied via `prisma migrate deploy` after a `--create-only` generation, specifically to avoid `prisma migrate dev`'s automatic seed re-run touching the shared dev DB's accumulated data — same caution this scenario has used throughout for every live DB change.
- Batch quality hold's shared-permission design (no real maker≠checker split) was surfaced to the developer explicitly and left as-is by their own choice — worth revisiting alongside Scenario 21 if/when a real two-tier gate is wanted there.
- Purchase Order has no `reject()` — only `approve()`/`cancel()`. `cancel()` was deliberately NOT wired to notify this pass (its semantics — a withdrawal, not necessarily an approver's rejection — are different enough from the create→approve pair covered here that it felt like separate scope); worth a follow-up ask if the developer wants it too.
- Multi-branch Price Lists (2+ `allowedBranchIds`) don't get a single-branch `Notification.branchId` (it's `null` for anything but exactly 1 branch) — consistent with the batch-hold multi-branch case, but means neither can be filtered to "my branch's alerts only" as cleanly as every other single-branch workflow in this catalog. Not fixed — `Notification.branchId` is a single nullable field by design; a real fix would mean either a join table or accepting the current "null = enterprise-or-multi-branch" convention permanently.

**Live-testing follow-up fixes (same day, after the above landed):**

- The 4 new workflows' 10 new `NotificationType` values were never added to the frontend's `NOTIFICATION_TYPE_META` map — clicking any of their notifications crashed (`meta` undefined in `resolveNotificationStatus`). Backend-only work in the round above; the frontend half was missed entirely. Fixed: added the type union members, icons, statuses, and real click-through routes (`/pos/reservations`, `/inventory/price-lists`, `/inventory/batches`, `/inventory/purchase-requests`, `/inventory/purchase-orders`) for all 10. `NOTIFICATION_TYPE_META`'s `Record<NotificationType, ...>` typing now makes this class of gap a compile error, not a runtime crash, going forward.
- Found while fixing the above: none of the 5 new `*_resolved` notify calls set `metadata: { approved }` like the existing ones (release-form, return-refund, credit application, item master, stock adjustment) do — a rejected outcome would have rendered as green/"approved" in the UI. Fixed all 5 (`sku_reservation_cancel_resolved`, `price_list_resolved`, `batch_hold_resolved`, `purchase_request_resolved`, `purchase_order_resolved`).
- The same "stale list across browser tabs" gap this scenario already hit 3 times in the original Phase 1+2 pass (credit applications, item master, stock adjustments) recurred in all 5 new workflows' own list hooks (`usePurchaseRequests`, `useSkuReservations`, `usePriceLists`, `useBatchManager`, `usePurchaseOrders`) — `staleTime` with no `refetchInterval`, so a different actor's submit/approve/reject in another tab never appeared without a manual refresh. All 5 fixed the same way (`refetchInterval: 10 * 1000`), swept proactively across all 5 once the pattern was reported on Purchase Request specifically (a new PR not appearing on the Owner's side, and an approval not reflecting back on the submitter's side, turned out to be the same one missing setting).
